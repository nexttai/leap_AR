...
namespace Leap.Unity.AR {
  ...
  public class WindowOffsetManagerEditor : CustomEditorBase<WindowOffsetManager> {
    ...
    void LayoutViews(System.Type GameViewType, bool isCalibrationView = false) {
      ...
      //変更必要なし？
      if (isCalibrationView) {
        SendViewToScreen(view, new Vector2Int(-2160, 0), new Vector2Int(2160, 1080*2 ));
      }
      //LCDの解像度変更。
            else {
        SendViewToScreen(view, WindowOffsetManager.WindowShift, new Vector2Int(2160, 1080 *2));
      }
    }
    ...
    void EnforceViewPositions() {
      EditorApplication.delayCall += () => {
        if (calibrationView != null) { calibrationView.position = new Rect(new Vector2Int(-2160, 0) + Vector2Int.down * (UNITY_MENU_HEIGHT_POSITION), calibrationView.position.size); }

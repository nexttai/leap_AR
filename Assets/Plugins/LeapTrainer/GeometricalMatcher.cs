...
public class GeometricalMatcher : TemplateMatcher
{
	...
	*/
    ...
    public int pointCount = 6;            // Gestures are resampled to this number of points
     ...
     *   // これは、検出された*ジェスチャーを既知のジェスチャーと比較するために上記のLeapTrainer
     *   // .Controllerで呼び出される主要な相関関数です。
	 * 
	 * @param gesture
 ...
 *    // 均一な長さのジェスチャを作成するために、ジェスチャをリサンプリングします。 2番目の
 *    // パラメータは、ジェスチャをリサンプリングする長さを*に指定します。 * *この機能は、ジ
 *    // ェスチャーの長さを均一にし、それらをより同等にするために使用されます。
 * 
 * @param gesture
 * @param newLength
 * @returns {Array}
 */
    List<Point> resample(List<Point> gesture, float newLength)
    {
        ...
        else target = newLength;
        Point p, pp, q;
        //距離分に縮小したいカウント24を割って、繰り返しの数を求める。(間隔を求めている。）
     ...
     */// 異なる縮尺で同じジェスチャを検出できるように、ジェスチャを均一な分散に調整します
        // 。
	 /* 
	 * @param gesture
	 * @returns {Array}
     ...
     *   // ジェスチャーを提供された重心に変換します。この機能は、同じであるが空間の異なる点
     *   // で発生するジェスチャを認識するために、すべてのジェスチャを*原点にマッピングするた
     *   // めに使用されます。
	 * 
	 * @param gesture
	 * @param centroid
